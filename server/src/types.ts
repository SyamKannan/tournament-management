...
export interface Plan {
  ...
  ad_limit: number;
  features: string[]; // ['live_scoring', 'scoreboard_tv', 'break_ads', 'emergency_announcements', 'pdf_exports', 'advanced_analytics']
...
export interface User {
  ...
  role: UserRole;
  avatar: string;
  organization_id?: string; // If associated with an org
  created_at: string;
  updated_at: string;
}
...
export interface Invoice {
  ...
  payment_method: 'stripe' | 'razorpay' | 'bank_transfer' | 'upi' | 'cash';
...
export interface TournamentSettings {
  ...
  match_duration_minutes?: number; // total duration
...
export interface Tournament {
  ...
  payment_config: {
    ...
    min_partial_value: number; // e.g. 50 (for 50%) or 2500
  };
  prize_money: number;
  ...
  has_auction?: boolean; // Controls whether this tournament has a player auction or direct team registration
  auction_id?: string;
...
export interface CricketDelivery {
  ...
  match_id: string;
  innings: 1 | 2;
  over_number: number; // 0-indexed over count or 1-indexed
  ball_number: number; // 1 to 6 valid balls
  bowler_id: string;
  striker_id: string;
  ...
  extras_runs: number;
  is_wicket: boolean;
  wicket_type?: 'bowled' | 'caught' | 'lbw' | 'run_out' | 'stumped' | 'hit_wicket' | 'caught_and_bowled' | 'retired_hurt' | 'obstructing_field';
